...
export default function MangaPage() {
    const { redirectedManga } = useManga();

    // Read from localStorage first, before render
    const storedManga = localStorage.getItem("lastChosenManga");
    const manga = redirectedManga || (storedManga ? JSON.parse(storedManga) : null);
    ...
    return (
        <div className="pb-10 px-4 md:px-10 text-black">
            <div className="mb-4 text-sm sm:text-base">
                <Link to="/mangas" className="text-black hover:text-gray-700">
                    &larr; Back to Manga List
                </Link>
            </div>

            <h1 className="text-4xl sm:text-5xl font-bold my-6">{manga.title}</h1>

            <div className="flex flex-col md:flex-row gap-6">
                <img
                    src={manga.image || "https://dennymfg.com/cdn/shop/products/ckgrayHigh_grande.jpg?v=1619109728"}
                    alt={manga.title}
                ...
                <div className="flex-1 flex flex-col gap-3">
                    ...
                    <p>
                        ...
                        <span className={manga.status === "completed" ? "text-black" : "text-red-500"}>
