const ShowsCard = ({ show }) => {
  return (
    <div className="w-60 bg-[#111] border border-gray-700 shadow-[0_0_15px_rgba(255,255,255,0.05)] rounded-lg overflow-hidden transition-transform duration-300 hover:scale-[1.02]">
      ...
      />
      <div className="px-4 py-3 flex flex-col gap-2 font-mono text-white">
        ...
        <p className="flex flex-wrap gap-1 text-xs text-gray-300 mt-1">
          Genres:&nbsp;
          {show.genres && show.genres.length > 0 ? (
            show.genres.map((genre, index) => (
              <span
                key={index}
                className="border border-gray-500 text-gray-200 text-[11px] px-2 py-0.5 rounded-full tracking-wide uppercase"
              >
                {genre}
              </span>
            ))
        ...
        {show.comment && (
          <p className="text-sm font-bold italic text-gray-300 mt-2">
            “{show.comment}”
          </p>
        )}
      </div>
